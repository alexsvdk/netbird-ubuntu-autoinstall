...
Установщик сопоставляет каждый заданный короткий `ID_SERIAL_SHORT` с единственным полным `ID_SERIAL` до запуска Curtin. Модельный префикс QEMU или физического диска не настраивается вручную. При отсутствии или неоднозначности соответствия установка останавливается до форматирования.
