...
# Префикс serial для QEMU или физического диска не нужен: установщик
# сам сопоставляет короткий ID с единственным полным ID_SERIAL.
...
Host-инстанс работает без `network_mode: host` и публикует только `127.0.0.1:7890` и `127.0.0.1:9090`. TUN для него отключён; полный TUN используется отдельным VPN-sidecar.
